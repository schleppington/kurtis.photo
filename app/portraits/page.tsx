...
export default function PortraitsPage() {
  ...
  const heroPhoto = heroPhotos[0]?.photo;
  if (heroPhoto) {
    preload(heroPhoto.variants["768"], {
      as: "image",
      fetchPriority: "high",
      imageSizes: "(max-width: 780px) 50vw, 33vw",
      imageSrcSet: buildPhotoSrcSet(heroPhoto),
    });
  }
    ...
    <section className="portrait-editorial-hero">
      <div className="portrait-hero-collage">
        {heroPhotos.map(({ collection, layout, photo }, index) => <figure className={`portrait-hero-frame is-${layout}`} key={photo.id}>
          <PhotoFrame
            alt={siteCopy.portraits.portraitAlt(collection.title)}
            aspectRatio="auto"
            fetchPriority={index === 0 ? "high" : "auto"}
            frameClassName="portrait-hero-media"
